...
gtboundingboxpath='/Users/abhinavbadinehal/saiabhin_CSE666_Biometrics_assignment1/groundtruths/3_expanalysis_groundtruth.json'
...
# #load the model
#facialExpression = FER(mtcnn=True)
emopred={}
for key,values in boxes.items():
    ...
    predicted_class = DeepFace.analyze(face,actions = ['emotion'],enforce_detection=False)
    #predicted_class=facialExpression.detect_emotions(face)
    #topFeeling=facialExpression.top_emotion(face)
    topFeeling=predicted_class[0]['dominant_emotion']
    emopred[key]={'dominant_emotion':topFeeling,'region':values}
    # print(key)
    # print(topFeeling)
    cv2.rectangle(image, (x, y), (x+w, y+h), (0, 255, 0), 2)
    cv2.putText(image, str(topFeeling), (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    # Draw the predicted expression label on the image
    # cv2.putText(image, predicted_class, (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
...
for gtlabels, gtboxes in groundtruth.items():
    best_iou=0
    best_box=None
    for predlabels, predboxes in emopred.items():
        iou=get_iou(gtboxes['region'],predboxes['region'])
        if(iou>best_iou):
            best_iou=iou
            best_box=predboxes
    if(best_iou>iou_threshold):
        #if that is the box then checking whether both the emotions are matching or not
        #if(gtboxes['dominant_emotion']==best_box['dominant_emotion'][0]):
        if(gtboxes['dominant_emotion']==best_box['dominant_emotion']):
            truePositives+=1
    else:
        falseNegatives+=1
for predlabels, predboxes in emopred.items():
